...
public class Point implements Comparable<Point> {
    ...
    public int compareTo(Point that) {
        ...
        // at this point y0 == y1, comparing x0 and x1
        if (this.x < that.x) {
            return -1;
        }
    ...
    public Comparator<Point> slopeOrder() {
        ...
        // in case lambda functions are not allowed in course
    ...
    public static void main(String[] args) {
        ...
        Point i = new Point(15, 8);
        Point j = new Point(10, 7);

        Point kk = new Point(1234, 5678);
        Point ll = new Point(14000, 10000);
        ...
        assert b.compareTo(d) > 0;

        assert c.compareTo(e) == 0;

        assert b.compareTo(f) > 0;
        assert b.compareTo(g) < 0;

        assert (Math.abs(i.slopeTo(j) - 0.2) <= 0.000001); // i.slopeTo(j) 0.2;
        assert (Math.abs(kk.slopeTo(ll) - 0.33855553814820616) <= 0.000001);

        // Point[] points = new Point[] { a, b, c, d, e, f, g, i, j };
        // Arrays.sort(points);
